...
class ModelGarage:
    ...
    async def complete(self, _messages: List[ChatMessage], _model: Optional[str], _provider_id: Optional[str], _provider_preference: Optional[List[ProviderType]], **kwargs) -> LLMResponse:
        """
    ...
    async def stream(self, _messages: List[ChatMessage], _model: Optional[str], _provider_id: Optional[str], **kwargs) -> AsyncIterator[str]:
        """Stream chat completion tokens."""
        if not self._initialized:
            await self.initialize()
