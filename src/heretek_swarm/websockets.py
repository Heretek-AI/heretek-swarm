...
class WebSocketAuthManager:
    ...
    def generate_token(self, user_id: str, metadata: Optional[Dict]) -> str:
...
async def authenticate_websocket(_websocket: WebSocket, token: Optional[str]) -> Tuple[bool, Optional[str], Optional[str]]:
    """
...
async def agent_status_websocket(
    websocket: WebSocket,
    agent_id: Optional[str] = Query(None, description="Specific agent ID to monitor"),
    token: Optional[str] = Query(None, description="Authentication token")
):
    """
...
async def workflow_progress_websocket(
    websocket: WebSocket,
    workflow_id: Optional[str] = Query(None, description="Specific workflow ID to monitor"),
    token: Optional[str] = Query(None, description="Authentication token")
):
    """
...
async def send_agent_status_update(agent_id: str, status: str, current_task: Optional[str]):
    """
...
async def send_workflow_progress_update(workflow_id: str, current_node: str, phase: str, progress: int):
    """
...
async def send_agent_metrics_update(agent_id: str, phi: Optional[float], coherence: Optional[float], load: Optional[float], queue_size: Optional[int]):
    """
...
# Export router
__all__ = ["router", "manager", "send_agent_status_update", "send_workflow_progress_update", "send_agent_metrics_update"]