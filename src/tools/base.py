...
class ToolMetadata(BaseModel):
    ...
    source: str | None = Field(None, description="Source repository")
...
class ToolExecutionResult(BaseModel, Generic[TOutput]):
    ...
    output: Any | None = Field(None)
    raw_output: Any | None = Field(None)
    ...
    # Context
    input_data: dict[str, Any] | None = Field(None)
...
class BaseTool(ABC, Generic[TInput, TOutput]):
    ...
    async def run(
        ...
        try:
            ...
            self.metadata.last_used_at = datetime.now(UTC)
