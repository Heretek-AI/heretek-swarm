...
class LineageNode:
    ...
    def __init__(self, lineage: MessageLineage):
        ...
        self.parent: UUID | None = lineage.parent_message_id
...
class LineageTracker:
    ...
    def _cache_get(self, message_id: UUID) -> MessageLineage | None:
        """Get from cache"""
    ...
    async def get_lineage(self, message_id: UUID) -> MessageLineage | None:
    ...
    async def mark_delivered(self, message_id: UUID) -> bool:
        ...
        node = self._nodes.get(message_id)
        if node:
            node.lineage.delivered_at = datetime.now(UTC)
            return True
        return False
    ...
    async def mark_processed(self, message_id: UUID) -> bool:
        ...
        node = self._nodes.get(message_id)
        if node:
            node.lineage.processed_at = datetime.now(UTC)
            return True
        return False
    ...
    async def verify_integrity(self, message_id: UUID) -> bool:
        ...
        if lineage.parent_message_id and lineage.parent_message_id not in self._nodes:
            logger.warning(
