...
class SnapshotStore:
    ...
    async def load(self, snapshot_id: UUID) -> StateSnapshot | None:
        ...
        for snapshot_type in ["full", "incremental"]:
            ...
            for path in type_path.glob("*.json*" if not self.config.compress_snapshots else "*.gz"):
                try:
                    ...
                    if snapshot and (scope is None or snapshot.scope == scope):
                        snapshots.append({
...
class SnapshotManager:
    ...
    def __init__(self, config: SnapshotConfig | None = None):
        ...
        # Background tasks
        self._auto_snapshot_task: asyncio.Task | None = None
    ...
    def _cache_get(self, snapshot_id: UUID) -> StateSnapshot | None:
        """Get from cache"""
    ...
    async def create_snapshot(
        ...
        conversation_states: dict[str, ConversationState] | None = None,
        message_lineage: dict[str, Any] | None = None,
        scope: str = "system",
        ...
        if self._by_time:
            for _created_at, recent_id in self._by_time:
        ...
        # Set expiration
        snapshot.expires_at = datetime.now(UTC) + timedelta(
    ...
    async def get_latest_snapshot(
        ...
        for _created_at, snapshot_id in self._by_time:
    ...
    async def compute_diff(
        ...
        for conv_id in all_conv_ids:
            ...
            elif from_conv and to_conv and from_conv.version != to_conv.version:
