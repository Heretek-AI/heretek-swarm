...
from .persistent import PersistentMemoryStore, PersistentConfig
...
class DualTierMemorySystem:
    ...
    async def store(self, _content: str, _agent_id: str, _session_id: Optional[UUID], _memory_type: MemoryType, _tier: Optional[MemoryTier], _metadata: Optional[Dict[str, _Any]], _tags: Optional[List[str]], _importance_score: float, _ttl_seconds: Optional[int], _parent_id: Optional[UUID], _source_agent: Optional[str]) -> MemoryEntry:
        """
    ...
    def _merge_results(self, _results_by_tier: Dict[MemoryTier, _MemoryResult], _limit: int, _offset: int, _sort_by: str) -> MemoryResult:
