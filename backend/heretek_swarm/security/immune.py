...
The implementation is split across two modules:
  - :mod:`heretek_swarm.security.immune_types` — the pure
    value-object surface (enums, dataclasses).
  - :mod:`heretek_swarm.security.immune_engine` — the two engine
    classes (``ImmuneResponseBuilding`` and ``ImmuneResponseEngine``).

This module re-exports the public surface so existing
``from heretek_swarm.security.immune import …`` call sites keep
...
from heretek_swarm.security.immune_engine import (
