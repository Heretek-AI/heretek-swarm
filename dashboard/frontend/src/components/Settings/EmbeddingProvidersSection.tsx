 ...
 * 
 * Component for managing embedding provider configurations.
 * Allows users to add, edit, test, and remove embedding providers.
 */

import React, { useState, useCallback, useEffect } from 'react';
import { useToast } from '../UI/Toast';
import configurationApi, { EmbeddingProvider, EmbeddingProviderCreate } from '../../api/configuration';

interface EmbeddingProvidersSectionProps {
...
export function EmbeddingProvidersSection({ onProviderChange }: EmbeddingProvidersSectionProps) {
  ...
  const loadProviders = useCallback(async () => {
    try {
      setLoading(true);
      const data = await configurationApi.listEmbeddingProviders();
      setProviders(data);
    } catch (error) {
      toast.error('Failed to load providers', 'Could not fetch embedding providers');
    } finally {
      setLoading(false);
    }
  }, [toast]);
  ...
  const handleOpenForm = useCallback((provider?: EmbeddingProvider) => {
    if (provider) {
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      const providerData: EmbeddingProviderCreate = {
        provider_name: formData.provider_name,
        provider_type: formData.provider_type,
        base_url: formData.base_url,
        default_model: formData.default_model || undefined,
        embedding_dimensions: formData.embedding_dimensions ? Number(formData.embedding_dimensions) : undefined,
      ...
      if (editingId) {
        await configurationApi.updateEmbeddingProvider(editingId, providerData);
        toast.success('Provider updated', `${formData.provider_name} has been updated`);
      } else {
        await configurationApi.createEmbeddingProvider(providerData);
    ...
    try {
      ...
      if (result.success) {
        toast.success(
          ...
          `${providerName} responded in ${result.latency_ms.toFixed(0)}ms (${result.dimensions}d)`
        );
      } else {
  ...
  const handleProviderTypeChange = useCallback((type: string) => {
    ...
    setFormData(prev => ({
      ...
      embedding_dimensions: DEFAULT_DIMENSIONS[type] || prev.embedding_dimensions,
    }));
  }, []);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Configure embedding models for vector operations
          </p>
        </div>
        <button
                ...
                <div className="flex-1">
                  ...
                  <div className="mt-2 text-sm text-gray-400">
                    ...
                    {provider.embedding_dimensions && (
                      <span className="ml-4">{provider.embedding_dimensions} dimensions</span>
                    )}
                  </div>
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="e.g., my-openai-embeddings"
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData(prev => ({ ...prev, embedding_dimensions: parseInt(e.target.value) || '' }))}
                  placeholder="e.g., 1536"
                  className="w-full px-4 py-2 bg-gray-900 border border-gray-600 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500"
                />
                <p className="text-xs text-gray-500 mt-1">Output vector dimensions (e.g., 1536 for OpenAI)</p>
              </div>
...
export default EmbeddingProvidersSection;
